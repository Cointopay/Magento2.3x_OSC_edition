    ...
    public function execute(\Magento\Framework\Event\Observer $observer)
    {
		...
		if ($payment_method_code == 'cointopay_gateway') {
            ...
            if ($UrlRewriteCollection->getFirstItem()->getId()) {
                ...
                //$rewrite = $urlRewriteModel->load('checkout/onepage/success');
                //$rewrite->setRedirectType(0);
                $deleteItem->delete();
                //$urlRewriteModel->setStoreId($rewrite->getStoreId());
                //$urlRewriteModel->setIsSystem($rewrite->getIsSystem());
                //$urlRewriteModel->setIdPath($rewrite->getIdPath());
                //$urlRewriteModel->setTargetPath($rewrite->getTargetPath());
                //$urlRewriteModel->setRequestPath($rewrite->getRequestPath());
               ...
               // $deleteItem->delete();
              //throw new \Magento\Framework\Exception\LocalizedException(__($rewrite->getRedirectType()));
                //$this->_response->create()->setRedirect($baseUrl.'paymentcointopay/success')->sendResponse();
                //exit(0);
                //return $this;                 
            }
